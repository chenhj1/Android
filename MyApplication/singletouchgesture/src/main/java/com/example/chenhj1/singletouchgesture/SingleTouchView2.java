...
public class SingleTouchView2 extends View {
	 ...
	 * 旋转或者放大状态(多触点)
	 */
	public static final int STATUS_MUTI_ROTATE_ZOOM = 3;
	...
	private Point mPreMovePointF = new Point();
	private Point mCurMovePointF = new Point();
	...
	public SingleTouchView2(Context context, AttributeSet attrs) {
	...
	public SingleTouchView2(Context context, AttributeSet attrs, int defStyle) {
	...
	private void init(){
		...
		if(controlDrawable == null){
			controlDrawable = getContext().getResources().getDrawable(R.drawable.ctrl);
		}
	...
	protected void onDraw(Canvas canvas) {
		...
		if(isEditable){
			...
			mPath.lineTo(mRTPoint.x, mRTPoint.y);
			PathEffect effects = new DashPathEffect(new float[]{25,15},1);
	...
	private void transformDraw(){
		...
		//设置画该图片的起始点
		mMatrix.postTranslate(offsetX + mDrawableWidth / 2, offsetY + mDrawableHeight / 2);
	 ...
	 * 设置Matrix, 强制刷新
	 */
	private void mutiTransformDraw(){
		if(mBitmap == null)
			return;
		int bitmapWidth = (int)(mBitmap.getWidth() * mScale);
		int bitmapHeight = (int)(mBitmap.getHeight()* mScale);
		//Log.e("tag","w: h:"+bitmapWidth+ "  "+bitmapHeight);
		computeRect(-framePadding, -framePadding, bitmapWidth + framePadding, bitmapHeight + framePadding, mDegree);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if(!isInArea(new Point((int)event.getX(),(int)event.getY()))){
				return false;
			}
			...
			//Log.e("tag", "LT:" + mLTPoint.x + " " + mLTPoint.y + "curr: " + mPreMovePointF.x + " " + mPreMovePointF.y);
		...
		/*case MotionEvent.ACTION_POINTER_DOWN:
			mStatus = STATUS_MUTI_ROTATE_ZOOM;
			mScale = spacing(event);
			mDegree = rotation(event);
			midPoint(mid, event);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mStatus == STATUS_ROTATE_ZOOM) {
				...
				float bitmapToCenterDistance = (float)Math.sqrt((double)(halfBitmapWidth * halfBitmapWidth + halfBitmapHeight * halfBitmapHeight));
				...
				Point centerToProMove = new Point((mPreMovePointF.x - mCenterPoint.x), (mPreMovePointF.y - mCenterPoint.y));
				
				//center -> curMove 的向量  
				Point centerToCurMove = new Point((mCurMovePointF.x - mCenterPoint.x), (mCurMovePointF.y - mCenterPoint.y));
				...
				transformDraw();
			}else if(mStatus == STATUS_MUTI_ROTATE_ZOOM){
				float rotation = rotation(event) - mDegree;
				float newDist = spacing(event);
				float scale = newDist / mScale;
				mMatrixTemp.set(mMatrix);
				mMatrixTemp.setScale(scale, scale, mid.x,mid.y);// 缩放
				mMatrixTemp.postRotate(rotation, mid.x, mid.y);// 旋转
				...
				//Log.e("tag","STATUS_MUTI_ROTATE_ZOOM");
				/*mDegree = rotation(event) - mDegree;
				float newDist = spacing(event);
				mScale = newDist / mScale;
	...
	private int JudgeStatus(int x, int y){
		...
		Point controlPointF = new Point(mControlPoint);
	 ...
	 * @return
	 */
	private float distance4PointF(Point pf1, Point pf2) {
	...
	private boolean isInArea(Point point){
		double abp = triangleArea(mLTPoint,mLBPoint,point);
		double bcp = triangleArea(mLBPoint,mRBPoint,point);
		double dcp = triangleArea(mRBPoint,mRTPoint,point);
		double adp = triangleArea(mLTPoint,mRTPoint,point);
		double countSize = abp + bcp + dcp + adp;

		double areaSize = distance4PointF(mLTPoint,mRTPoint) * distance4PointF(mLTPoint,mLBPoint);

		//Log.e("tag","CountSize: "+countSize+"  areaSize: "+areaSize);

		//50000是容错，单位：平方像素。
		if((areaSize < (countSize + 50000)) && (areaSize > (countSize - 50000))){
			return true;
		}else {
			return false;
		}
